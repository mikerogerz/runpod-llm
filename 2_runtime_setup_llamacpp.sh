...
CMAKE_ARGS="-DGGML_CUDA=on -DGGML_CUDA_FA_ALL_QUANTS=ON" pip3 install --root-user-action=ignore "llama-cpp-python[server] @ https://github.com/JamePeng/llama-cpp-python/releases/download/v0.3.17-cu130-AVX2-linux-20251209/llama_cpp_python-0.3.17-cp313-cp313-linux_x86_64.whl" --no-cache-dir
...
ARGS="--model ${MODEL_PATH} --model_alias ${RUNPOD_LLM_MODEL_ALIAS} --embedding 1 --pooling 3"
...
if [ -n "${RUNPOD_LLM_FLASH_ATTENTION}" ]; then
    if [ "${RUNPOD_LLM_FLASH_ATTENTION}" = "1" ]; then
        ARGS="${ARGS} --flash_attn 1"
    else
        ARGS="${ARGS} --flash_attn 0"
    fi
fi
